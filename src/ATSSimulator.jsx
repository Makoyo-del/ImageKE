...
// ─── Status Badge ─────────────────────────────────────────────────────────────
function StatusBadge({ found, warningText }) {
...
export default function ATSSimulator({ onBack }) {
  ...
  // VIEW: RESULTS
  // ─────────────────────────────────────────────────────────────────────────
  if (view === 'results' && parseResult) {
    const { contact, sections, hasEducation, yearsExp, skills, hasAchievements, formatting, scores, recommendations } = parseResult;
    const overallScore = scores.overallScore;

    const scoreColor = overallScore >= 75 ? '#14B8A6' : overallScore >= 50 ? '#F59E0B' : '#EF4444';
    const scoreLabel = overallScore >= 85 ? 'ATS Ready' : overallScore >= 60 ? 'Needs Work' : 'Critical Issues';

    // Safe fallbacks for V1 / V2 mixed fields
    ...
    const formatCheck = {
      ...
      isLikelyScanned: formatting.isLikelyScanned,
      hasTables: formatting.hasTables,
      hasGraphics: formatting.hasGraphics || false,
      hasTextBoxes: formatting.hasTextBoxes || false,
      hasHeaderFooterContact: formatting.hasHeaderFooterContact || false,
      hasCreativeHeadings: formatting.hasCreativeHeadings || false,
      hasSpecialBullets: (formatting.specialCharCount > 10) || formatting.hasSpecialBullets || false,
      isOverTwoPages: formatting.isOverTwoPages || false,
      hasColoredTextOrBg: formatting.hasColoredTextOrBg || false,
    };

    const privacyCheck = {
      hasPhoto: contact.hasPhoto || false,
      hasDOB: contact.hasDOB || false,
      hasMaritalStatus: contact.hasMaritalStatus || false,
      hasIDNumber: contact.hasIDNumber || false,
      hasFullAddress: contact.hasFullAddress || false,
    };

    const educationFound = (sections && sections.education !== undefined) ? sections.education : hasEducation;

    let ctaHeadline, ctaBody;
    if (overallScore >= 85) {
      ctaHeadline = '🎉 Your CV is ATS-Friendly!';
      ctaBody = "Want a recruiter-level review to make it even stronger? Duncan will give you specific advice on how to increase your interview rate further.";
    } else if (overallScore >= 60) {
      ctaBody = "Your CV is readable, but a few issues could reduce your visibility. Duncan can rewrite these sections to maximize your chances.";
      ctaHeadline = '⚡ A Few Issues Could Cost You Interviews';
    } else {
      ctaHeadline = '🚨 Your CV Has Critical ATS Issues';
      ctaBody = "Most ATS systems will struggle to parse this CV correctly. Duncan specializes in rewriting CVs that pass ATS screening and get results.";
    }
                ...
                <table className="ats-table">
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td>{privacyCheck.hasPhoto ? <StatusBadge warningText="Risk — Remove photo" /> : <StatusBadge found={true} />}</td>
                    </tr>
                    <tr>
                      <td style={{ fontWeight: 600 }}>Date of Birth / Age</td>
                      <td>{privacyCheck.hasDOB ? <StatusBadge warningText="Risk — Remove age/DOB" /> : <StatusBadge found={true} />}</td>
                    </tr>
                    <tr>
                      <td style={{ fontWeight: 600 }}>Marital Status / Gender</td>
                      <td>{privacyCheck.hasMaritalStatus ? <StatusBadge warningText="Risk — Remove status" /> : <StatusBadge found={true} />}</td>
                    </tr>
                    <tr>
                      <td style={{ fontWeight: 600 }}>National ID / Passport No.</td>
                      <td>{privacyCheck.hasIDNumber ? <StatusBadge warningText="Critical Risk — Remove ID" /> : <StatusBadge found={true} />}</td>
                    </tr>
                    <tr>
                      <td style={{ fontWeight: 600 }}>Full Street Address</td>
                      <td>{privacyCheck.hasFullAddress ? <StatusBadge warningText="Use City, Country only" /> : <StatusBadge found={true} />}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
                ...
                <table className="ats-table">
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td>{formatCheck.multiColumnRisk ? <StatusBadge warningText="May scramble reading order" /> : <StatusBadge found={true} />}</td>
                    </tr>
                    <tr>
                      <td style={{ fontWeight: 600 }}>Layout tables used</td>
                      <td>{formatCheck.hasTables ? <StatusBadge warningText="Tables detected" /> : <StatusBadge found={true} />}</td>
                    </tr>
                    <tr>
                      <td style={{ fontWeight: 600 }}>Graphics / progress bars / icons</td>
                      <td>{formatCheck.hasGraphics ? <StatusBadge warningText="Graphics detected" /> : <StatusBadge found={true} />}</td>
                    </tr>
                    <tr>
                      <td style={{ fontWeight: 600 }}>Floating text boxes</td>
                      <td>{formatCheck.hasTextBoxes ? <StatusBadge warningText="Text boxes detected" /> : <StatusBadge found={true} />}</td>
                    </tr>
                    <tr>
                      <td style={{ fontWeight: 600 }}>Colored text / backgrounds</td>
                      <td>{formatCheck.hasColoredTextOrBg ? <StatusBadge warningText="Use black text on white paper" /> : <StatusBadge found={true} />}</td>
                    </tr>
                    <tr>
                      <td style={{ fontWeight: 600 }}>Page count under 2-page limit</td>
                      <td>{formatCheck.isOverTwoPages ? <StatusBadge warningText="Exceeds 2-page cap" /> : <StatusBadge found={true} />}</td>
                    </tr>
                    <tr>
                      <td style={{ fontWeight: 600 }}>Standard uppercase headings</td>
                      <td>{formatCheck.hasCreativeHeadings ? <StatusBadge warningText="Creative headers found" /> : <StatusBadge found={true} />}</td>
                    </tr>
                    <tr>
                      <td style={{ fontWeight: 600 }}>Standard bullet points</td>
                      <td>{formatCheck.hasSpecialBullets ? <StatusBadge warningText="Decorative bullets found" /> : <StatusBadge found={true} />}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
                ...
                <table className="ats-table">
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td>{expEval.usesStarMethod ? <StatusBadge found={true} /> : <StatusBadge warningText="Lacks STAR format" />}</td>
                    </tr>
                    <tr>
                      <td style={{ fontWeight: 600 }}>Quantified metrics & results</td>
                      <td>{expEval.hasMetrics ? <StatusBadge found={true} /> : <StatusBadge warningText="Lacks metrics/statistics" />}</td>
                    </tr>
                    <tr>
                      <td style={{ fontWeight: 600 }}>Bolds first 3-5 words of bullets</td>
                      <td>{expEval.boldsFirstWords ? <StatusBadge found={true} /> : <StatusBadge warningText="No bolded first phrase" />}</td>
                    </tr>
                    <tr>
                      <td style={{ fontWeight: 600 }}>Bolds numbers and percentages</td>
                      <td>{expEval.boldsMetrics ? <StatusBadge found={true} /> : <StatusBadge warningText="No bolded metrics" />}</td>
                    </tr>
                    <tr>
                      <td style={{ fontWeight: 600 }}>Proper capitalization & full stops</td>
                      <td>{expEval.grammarCapitalizationAndPeriods ? <StatusBadge found={true} /> : <StatusBadge warningText="Inconsistent endings" />}</td>
                    </tr>
                    <tr>
                      <td style={{ fontWeight: 600 }}>No raw asterisks in copy</td>
                      <td>{expEval.rawAsterisksFound ? <StatusBadge warningText="Asterisks found (e.g. *247#)" /> : <StatusBadge found={true} />}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
